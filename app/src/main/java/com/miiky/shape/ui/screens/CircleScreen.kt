...
import com.miiky.shape.methods.circle
...
fun CircleScreen(
	...
	val thickX = remember { mutableIntStateOf(5) }
	val thickY = remember { mutableIntStateOf(5) }

	var list = remember {
		circle(
			Cords(startX.intValue, startY.intValue),
			Cords(thickX.intValue, thickY.intValue)
		)
	}
	...
	fun update() {
		...
		list.clear()
		list =
			circle(Cords(startX.intValue, startY.intValue), Cords(thickX.intValue, thickY.intValue))
		selected.intValue = 0
	}
	Column(
		modifier = modifier
		...
		Row {
			NumberTextInput(number = thickX, label = "Diameter", modifier = Modifier.weight(1f))
			NumberTextInput(number = thickY, label = "thick", modifier = Modifier.weight(1f))
		}
		Row {
			Button(onClick = {
				update()
